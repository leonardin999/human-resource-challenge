...
complete employee details
    ...
    ${id}=    get Employee ID
    ${employee_data}=    get Employee Data    ${id}
...
get Employee Data
    ...
    FOR    ${item}    IN    @{hr_api_data}
        set to dictionary    ${employee_app_data}    ${item}    ${hr_api_data}[${item}]
    END
    RETURN    ${employee_app_data}

get HR api Data
    [Documentation]    Deserialize the Response information
    [Arguments]    ${id}
    ${json_data}=    get HR api response    ${id}
    &{dict_data}=    Create Dictionary    phone_number=${json_data}[phoneNumber]
...
get HR api response
    ...
    [Arguments]    ${id}
    ${api_url}=    Set Variable    ${HR_API_URL}?id=${id}
...
Confirmation dialog
    Add icon    Warning
    Add heading    Check Enough Field?
    Add submit buttons    buttons=No,Yes    default=Yes
    ${result}=    Run dialog
    IF    $result.submit == "Yes"    Click Button    css:#submitButton

take screenshot of results
    [Documentation]    Take screenshot after filll information
    Wait Until Element Is Visible    css:.modal-confirm
    RPA.Browser.Selenium.Screenshot    filename= ${CURDIR}\output\result.png
